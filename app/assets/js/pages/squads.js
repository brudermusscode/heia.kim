...
$(function () {
   ...
   */
  $(document).on("submit", '[request-do="squad/post/comment/create"]', function (e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.status) {
          ...
          post?.find("[comments]").insertAdjacentHTML("afterbegin", data.data);
   ...
   */
  $(document).on("click", '[data-action="squad:post:vote:create"]', function (e) {
    e.preventDefault();
      ...
      success: function (data) {
        ...
        if (data.status) {
          ...
          if (vote_count) {
            ...
            else if (new_vote_count < 0) vote_count.setAttribute("color", "red");
            else {
   ...
   * @controller SquadPostVotesController
   */
  $(document).on("click", '[data-action="squad:post:vote:delete"]', function (e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.status) {
          ...
          if (vote_count) {
            new_vote_count = parseInt(vote_count.innerHTML.replace(",", "")) - type;
            vote_count.innerHTML = new_vote_count.toLocaleString("en-US");
