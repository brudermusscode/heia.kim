...
class Upload
{
  ...
  /**
   * Checks the file array's error key for a positive value and if a tmp_name key is
   * set.
   *
   * @param array $file
   * @return void
   ...
   */
  public static function error(array $file)
  {

    if (empty($file["tmp_name"]))
      die(error("Invalid file chosen."));

    if (!empty($file["error"]))
      die(error(match ($file["error"]) {
        2,
        ...
        default => "<strong>Unknown error.</strong>"
      }));
  }
}
