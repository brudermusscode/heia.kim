>[!Caution]
>It's in a pre-refactored state, so alot of things are still broken.

# heia.kim

It's the web base for https://www.heia.kim. Just for transparency,<br>
you can view the code! I was coding all alone in the hidden, push-<br>
ing to my own secret git repository when suddenly I had a feeling<br>
of being so full of myself. It should be public, so we can enjoy<br>
code together 🫶

<br>

>[!Note]
><strong>Copyright (c) Justin Seidel</strong> - All Rights Reserved (2022-now)<br>
It's a project of heart and I have put alot of time into it.<br>
If you really want to copy it, use it for a good reason and<br>
try to make something new, something that belongs to <br> 
you out of it 🙂

<br>
<br>

<p align=right><strong>Jesus loves you my friend ❤️</strong></p>
